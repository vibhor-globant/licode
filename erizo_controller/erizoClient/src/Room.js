...
Erizo.Room = function (spec) {
    ...
    sendDataSocket = function (stream, msg) {
        if (stream.local) {
            sendMessageSocket("sendDataStream", {
                id: stream.getID(),
                msg: msg
            });
        } else {
    ...
    updateAttributes = function (stream, attrs) {
        if (stream.local) {
            ...
            sendMessageSocket("updateStreamAttributes", {
                id: stream.getID(),
                attrs: attrs
            });
        } else {
    ...
    connectSocket = function (token, callback, error) {
        ...
        that.socket.on('publish_me', function (arg) {
            ...
            myStream.pc[arg.peerSocket] = Erizo.Connection({
                ...
                turnServers: that.turnServers,
                turnOnly: spec.turnOnly,
                pcUpdate: spec.pcUpdate
            });
        ...
        that.socket.on('disconnect', function (argument) {
            ...
            if (that.state !== DISCONNECTED) {
                var disconnectEvt = Erizo.RoomEvent({
    ...
    that.startRecording = function (stream, callback) {
        ...
        sendMessageSocket('startRecorder', {
            to: stream.getID()
        }, function (id, error) {
            if (id === null) {
    ...
    that.stopRecording = function (recordingId, callback) {
        sendMessageSocket('stopRecorder', {
            id: recordingId
            ...
            if (that.localStreams.hasOwnProperty(index)) {
                ...
                if (stream.hasOwnProperty("pc")) {
                    ...
                    if (stream.pc.hasOwnProperty("getStats")) {
                        promises.push(_getStats(streamId, "", stream.pc, "local"));
                    } else {
                        for (jj in stream.pc) {
